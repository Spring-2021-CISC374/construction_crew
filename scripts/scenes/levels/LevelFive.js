class LevelFive extends Phaser.Scene {
  constructor() {
    ...
    create() {
      ...
      var instructions = this.add.text(0, 0, "Level 5! Do you hear all that construction noise?!\n" +
      "\n\nIt looks like there's going to be a storm this week! Oh no!" +
      "\n Be sure to schedule your subcontractors so they are still in the " +
      "\ncorrect order AND they don't get caught in the bad weather!", {
        font: "35px Arial",
        fill: "black",
        align: "center"
      });
      ...
      */
      ...
      const storm2 = this.add.image(config.width/2 + 80, config.height/2 - 190, "storm")
      storm2.scale = 0.15

      const storm3 = this.add.image(config.width/2 + 160, config.height/2 - 190, "storm")
      storm3.scale = 0.15
    ...
    updateToCalendar() {
      const correct = true;
      const leveler = "LevelFive";
      //This needs to be changed
      this.scene.start("Calendar", {contractor: ["Concrete", "Framer", "Plumber", "Roofer", "Electrician", "Painter"], blocked: [2], level: 5, rows: 2, weather: [5,1,3]});
    }
  }
